...
function my_custom_init() {
	...
	foreach($posttypes as $foo) {
		add_post_type_support( $foo, 'excerpt' );
	}
}
...
function showshareingpost($url,$img, $title, $code=false,$top=false) { 
	...
	if(of_get_option('md_social_post_facebook')) {
		if(!$code) {
	$output .= '<div class="facebook shr"><iframe src="//www.facebook.com/plugins/like.php?href='.urlencode($url).'&amp;send=false&amp;layout=button_count&amp;width=65&amp;show_faces=false&amp;action=like&amp;colorscheme=light&amp;font&amp;height=21" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:78px; height:21px;" allowTransparency="true"></iframe></div>';
