...
?>  
	...
	a.main-logo { 
		font-family:<?php if(isset($hfont['face']) && $hfont['size']!='default') { echo $hfont['size']; }else{ echo $scolors['body_font']; }?>, "HelveticaNeue", "Helvetica Neue", Helvetica, Arial, sans-serif;
...
?>
    <?php 
		if(of_get_option('md_menu_home_url')) { 
		 ...
		 if(of_get_option('md_header_logo')) { 
			$logoshow = '<a href="'.$homeurl.'" class="main-logo pull-left" title="'.get_bloginfo( 'name' ).'"><img src="http://new.studiomova.me/wp-content/themes/mova/images/logo'.$randomnumber.'.png" class="" alt="'.get_bloginfo( 'name' ).'"></a>';
	...
	?>
                		 ...
                		 <div class="<?php if($md_home_full) { echo 'eight'; }else{ echo 'five';}?> columns omega pull-right toprightshare socialicons <?php if($md_home_full) { echo 'smalldontshow'; }else{ echo 'smalldevicedontshow'; } ?>">
							  ...
							  <?php 
                                if($md_home_full) { 
								 ...
								 <?php if(!of_get_option('md_social_top_hide')) { showSharing(); }?>
            ...
            <div class="row <?php if(of_get_option('md_removeheader')==1) echo 'removeheadermargin'; ?>">
            	<div class="three-sp columns main_leftmenu mobiledontshow <?php if(($md_works_full==1 && is_any_single('works')) || ($md_home_full==1)) echo 'hideit';?>">
